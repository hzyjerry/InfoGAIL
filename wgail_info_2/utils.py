...
def get_state(img, info, feat_dim, aux_dim, feat_extractor):
    img[:, :, [0, 2]] = img[:, :, [2, 0]]
    ...
    img = (np.array(img, dtype=np.float32) - 128.) / 128.
...
def rollout_contin(env, agent, feat_extractor, feat_dim, aux_dim, encode_dim,
    ...
    for p in range(paths_per_collect):
        print("Rollout index:", p, "/", paths_per_collect)
        feats, auxs, encodes, imgs, raws, actions, logstds = \
                [], [], [], [], [], [], []

        img, info = env.reset()
        feat, aux, img = get_state(img, info, feat_dim, aux_dim,
                                   feat_extractor)
